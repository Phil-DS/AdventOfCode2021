...
for i in range(1,len(data)):
    ...
    for p in range(len(path)-1):
        ...
        vect = np.dot(edge['rot'],vect) - edge['diff']
    vects.append(vect)

dist = 0

for point1,point2 in itertools.combinations(range(len(data)),2):
    dist = max(dist, np.sum(np.abs(vects[point2]-vects[point1])))

print('Largest Distance:',dist)     
